...
- **규칙**: 기본 3(상한 99), 리셋 -1, 광고 시청 +3(판 안에선 99까지 스택), **판 클리어 시 무조건 3으로 리셋**
...
- [ ] iOS v1.0.2 빌드 & App Store 제출 — code_index 저장 로직 + **activeInputHandler=Both 복구** + 랜덤 셔플 + 캡처 방지 + 강제 업데이트 + AdManager 광고 카운트 수정 + UpdateChecker [종료] + 목숨 시스템 + 셔플 그룹 확장 + **RewardManager v1.0.8 재작성 + 설정창 인증코드 보기** 반영. iOS에서 Application.Quit이 심사에 문제되는지 리젝 사례 사전 조사 권장
